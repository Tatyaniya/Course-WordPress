$(function() {
    ...
    $('.minus_popup').click(function () {
		var $input = $(this).parent().find('input');
        var count = parseInt($input.val()) - 1;
        var $final = $('#final_popup_price');
        var $link = $('#add_popup_cart');

        
        count = count < 1 ? 1 : count;

        if ($(this).data('price') < parseInt($final.text())) {
            $final.text(parseInt($final.text()) - $(this).data('price'));
        }

        $input.val(count);
        $link.attr('href', $(this).data('link') + '&quantity=' + $input.val());
		$input.change();
		return false;
	});
   ...
   /* function getUrlVars() {
    ...
    // Отключить перезагрузку и инитить аякс
    
	// $('.sort-menu li').on('click',function() {
           // $('#parametr').html($(this).find('a').html());

           // let link = $(this).find('a').data('value');
           // window.location = 'http://localhost/wordpress/shop/?orderby=' + link;
            /*window.location = 'http://jc.tatyaniya.com/shop/?orderby=' + link;*/
...
$(document).ready(function(){
	...
	/*$('[data-submit]').on('click', function(e){
		e.preventDefault();
		$(this).parents('.subscribe__form, .cabinet__form, .review-form, .subscr__form, .order-form, .callme-form').submit();
	})*/
	$.validator.addMethod(
		"regex",
	...
	$('form.subscribe__form, .cabinet__form, .review-form, .subscr__form, .order-form, .callme-form').each(function() {
		valEl($(this)); 
	});
});
