...
//если ф-ция не задекларирована, то мы ее создаем
if (!function_exists('wayup')) {
	function wayup( $atts, $content = null ) {
		extract(shortcode_atts(array(
			'title'   => 'не задано'
	    ), $atts));
		
       //return '<div class="ale-alert '.$title.'">' . do_shortcode($content) . '</div>';
       return '<h1>' . $title . '</h1> <div>' . do_shortcode($content) . '</div>';
	}
	add_shortcode('wayup', 'wayup');
}
...
if (!function_exists('ale_map')) {
    ...
    function ale_map_load_scripts() {
        wp_register_script( 'google-maps-api', 'http://maps.google.com/maps/api/js?key=AIzaSyCyy-sYVmab0g78HupUx2oQpl5mkZZfddA' );
    }
