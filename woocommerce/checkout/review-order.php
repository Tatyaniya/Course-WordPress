...
<div class="shop_table woocommerce-checkout-review-order-table table">
        ...
        foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
            ...
            if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_checkout_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
                ...
                <div class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?> row">
                    ...
                    <!-- если то первый элемент, показываем "товар" -->
                    ...
                    <!-- если то первый элемент, показываем "Итого" -->
    ...
    <div class="row">
		...
		<?php if ( wc_tax_enabled() && ! WC()->cart->display_prices_including_tax() ) : ?>
			<?php if ( 'itemized' === get_option( 'woocommerce_tax_total_display' ) ) : ?>
				<?php foreach ( WC()->cart->get_tax_totals() as $code => $tax ) : ?>
					<div class="tax-rate tax-rate-<?php echo sanitize_title( $code ); ?> row">
        ...
        <?php do_action( 'woocommerce_review_after_before_order_total' ); ?>
